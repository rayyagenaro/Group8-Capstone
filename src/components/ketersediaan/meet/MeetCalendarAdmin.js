// src/components/ketersediaan/meet/MeetCalendarAdmin.js
...
export default function MeetCalendarAdmin({ roomId, styles }) {
  ...
  const fetchMonth = useCallback(async (y_m) => {
    ...
    setLoading(true);
    try {
      // ⬇⬇⬇ PENTING: gunakan route ini, BUKAN /api/bimeet/calendar
      const res = await fetch(
        `/api/ketersediaanAdmin?type=bimeet_calendar&roomId=${roomId}&month=${y_m}&t=${Date.now()}`,
        { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' }, credentials: 'include' }
      );
      ...
      console.error('MeetCalendarAdmin fetchMonth error:', e);
      alert('Gagal memuat kalender ruangan');
    } finally {
      setLoading(false);
    }
  }, [roomId]);
  ...
  const toggleSlot = async (dateObj, time) => {
    ...
    const ok = confirm(
      adminBlocked
        ...
        : `Tutup slot ${time} pada ${dateObj.toLocaleDateString('id-ID')} untuk pemesanan?`
    );
    if (!ok) return;
    ...
    try {
      const res = await fetch('/api/ketersediaanAdmin', {
        ...
        credentials: 'include',
        body: JSON.stringify({ type: 'bimeet_calendar', action, roomId, bookingDate: dateStr, slotTime: time })
      });
        ...
        {matrix.map((week, wi) => (
          <React.Fragment key={wi}>
            {week.map((d, di) => {
              ...
              const roomOpen = sessionsToday.length > 0;
              ...
              return (
                <div key={`${wi}-${di}`} className={`${styles.calCell} ${inMonth ? '' : styles.calCellMuted}`}>
                  <div className={styles.calCellHeader}>
                    ...
                    {roomOpen && <span className={styles.calBadgeOpen}>Buka</span>}
                  </div>
