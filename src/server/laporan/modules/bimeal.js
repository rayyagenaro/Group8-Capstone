...
export const excel = {
  ...
  columns: [
    ...
    { header: 'Menu Dipesan',       key: 'menu_dipesan',       width: 40 },
  ],
...
export async function preview({ db, fromYMD, toYMD }) {
  ...
  const sql = `
    SELECT
      ...
      b.created_at,
      b.updated_at,
      GROUP_CONCAT(CONCAT(i.nama_pesanan, ' (', i.jumlah, ' ', i.satuan, ')') SEPARATOR ', ') AS menu_dipesan
    FROM bimeal_bookings b
    LEFT JOIN unit_kerja u ON b.unit_kerja_id = u.id
    LEFT JOIN bimeal_booking_items i ON b.id = i.booking_id
    ${whereSQL}
    GROUP BY b.id
    ORDER BY b.id ASC
    LIMIT 10000
